/*
    Code Break Database Connection manager
...
//   private PreparedStatement snapProjectQuery;
    ...
    public DatabaseConnectionManager(CodeBreakServer mcs, Properties p, Connection dbconn) {
        super(mcs, p, false);
        con = dbconn;
    ...
    private void initQueries() {
        try {
            findProjectsByHashQuery = con.prepareStatement("select p.pid,p.hash,p.gpid,p.description,f.parent,p.snapupdateid,q.description,p.pub,p.sub,p.owner from projects p left join (forklist f left join projects q on f.parent=q.pid) on p.pid = f.child where p.hash = ? order by p.pid asc;");
            //findProjectByPidQuery = con.prepareStatement("select pid,hash,gpid,snapupdateid,description from projects where pid = ?;");
            findProjectByPidQuery = con.prepareStatement("select p.pid,p.hash,p.gpid,p.snapupdateid,p.description,f.parent,q.description,p.pub,p.sub,p.owner from projects p left join (forklist f left join projects q on f.parent=q.pid) on p.pid=f.child where p.pid = ? order by p.pid asc;");
            findProjectByGpidQuery = con.prepareStatement("select pid,hash,gpid from projects where gpid = ? order by pid asc;");
            ...
            if (useMysql) {
                ...
                addProjectQuery = con.prepareStatement("insert into projects (hash,gpid,description,owner,pub,sub) values (?,?,?,?,?,?) returning pid;");
                addProjectSnapQuery = con.prepareStatement("insert into projects (hash,gpid,description,owner,snapupdateid) values (?,?,?,?,?) returning pid;");
